...
def client_compute(Input):

    c_model.eval()
    hidden = c_model(Input)
...
def server_compute(Hidden, target, only_forward = False):
    # Hidden = torch.tensor(Hidden, requires_grad=True).float()
    label = torch.argmax(torch.from_numpy(target).float()).view(1,)
    s_model.train()
    c_model.train()
    s_optimizer.zero_grad()
    c_optimizer.zero_grad()
    input = c_model(Hidden)
    input.retain_grad()
...
def sendSample(device, samplePath, num_button, deviceIndex, only_forward = False):
    ...
    with open(samplePath) as f:
        ...
        if only_forward:
            ...
            hidden_activation = torch.from_numpy(convert_string_to_array(outputs)).float()
            hidden_activation = hidden_activation.view(6,24,8).permute(2, 0, 1)
            label = convert_string_to_array(str(nb), one_hot = True)
            # print("hidden_activation Shape:", hidden_activation.shape)
            # print("hidden_activation Value:", hidden_activation)


            input_array = torch.from_numpy(input_array).float().view(1,13,50)
            gt_hidden = client_compute(input_array)
            # print("ground-truth hidden_activation Shape:", gt_hidden.size())
            # print("ground-truth hidden_activation Value:", gt_hidden)


            input_array = input_array.view(1,1,13,50)
            forward_accu, forward_error, error_array = server_compute(input_array, label, only_forward= False)
            ...
            # weight_old = torch.flatten(c_model.state_dict()['client.0.weight']).clone()

            # devices_hidden_layer_old = np.empty((size_hidden_layer, ), dtype='float32')

            # for i in range(size_hidden_layer): # hidden layer
            #     data = d.read(4)
            #     [float_num] = struct.unpack('f', data)
            #     devices_hidden_layer_old[i] = float_num
            ...
            # devices_hidden_layer = np.empty((size_hidden_layer, ), dtype='float32')

            # for i in range(size_hidden_layer): # hidden layer
            #     data = d.read(4)
            #     [float_num] = struct.unpack('f', data)
            #     devices_hidden_layer[i] = float_num
            # print("hidden weight Update Value:", devices_hidden_layer - devices_hidden_layer_old)

            # print("True weight update value:")
            # print(torch.flatten(c_model.state_dict()['client.0.weight']) - weight_old)
            # # print()

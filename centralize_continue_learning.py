...
# source_experiment = "./saved_results/EN_digits/large_conv2d_model_momentum0.6"
source_experiment = "./saved_results/EN_digits/small_fc_model"
experiment = 'digits' # 'iid', 'no-iid', 'train-test', 'custom', 'digits'
# model_type = "fc"
if "large_conv2d" in source_experiment:
    model_type = "conv2d"
else:
    model_type = "fc"
...
if model_type == "conv2d" and experiment == "digits":
    #best
    momentum = 0.6
    learningRate= 0.005
    number_hidden = 1
...
if model_type == "fc" and experiment == "digits":
    #best
    momentum = 0.6
    learningRate= 0.0005
    number_hidden = 0
...
logger.debug(f"Perfom Continue Learning on {experiment} Task, using model saved in {source_experiment}.")
...
c_model.load_state_dict(torch.load(source_experiment + "/c_model.pt"))
s_model.load_state_dict(torch.load(source_experiment + "/s_model.pt"))
...
if experiment == "digits":
    CN_digits_silence_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("silence") and int(file.split(".")[1])<500]
    CN_digits_one_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("one") and int(file.split(".")[1])<500]
    CN_digits_two_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("two") and int(file.split(".")[1])<500]
    CN_digits_three_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("three") and int(file.split(".")[1])<500]
    CN_digits_four_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("four") and int(file.split(".")[1])<500]
    CN_digits_five_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("five") and int(file.split(".")[1])<500]
    CN_digits_unknown_files = [file for file in os.listdir("datasets/CN_digits") if file.startswith("unknown") and int(file.split(".")[1])<500]

if experiment == "EN_digits" or "EN_digits" in source_experiment:
    EN_digits_silence_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("silence") and int(file.split(".")[1])>=500]
    EN_digits_one_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("one") and int(file.split(".")[1])>=500]
    EN_digits_two_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("two") and int(file.split(".")[1])>=500]
    EN_digits_three_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("three") and int(file.split(".")[1])>=500]
    EN_digits_four_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("four") and int(file.split(".")[1])>=500]
    EN_digits_five_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("five") and int(file.split(".")[1])>=500]
    EN_digits_unknown_files = [file for file in os.listdir("datasets/EN_digits") if file.startswith("unknown") and int(file.split(".")[1])>=500]



random.shuffle(CN_digits_silence_files)
random.shuffle(CN_digits_one_files)
random.shuffle(CN_digits_two_files)
random.shuffle(CN_digits_three_files)
random.shuffle(CN_digits_four_files)
random.shuffle(CN_digits_five_files)
random.shuffle(CN_digits_unknown_files)

random.shuffle(EN_digits_silence_files)
random.shuffle(EN_digits_one_files)
random.shuffle(EN_digits_two_files)
random.shuffle(EN_digits_three_files)
random.shuffle(EN_digits_four_files)
random.shuffle(EN_digits_five_files)
random.shuffle(EN_digits_unknown_files)
...
def raw_to_mfcc(filename, experi_name):
        # with open('./datasets/CN_digits/'+filename) as f:
    with open(f'./datasets/{experi_name}/'+filename) as f:
        data = json.load(f)
        if 'payload' in data:
...
def getSamplesIIDDigits(batch_size, batch_start_index, experi = "EN_digits"):
    # global EN_digits_silence_files, digits_one_files, digits_two_files, digits_three_files, digits_four_files, digits_five_files, digits_unknown_files
    # each_sample_amt = int(batch_size/2)
    ...
    for i in range(real_start, real_end):
        if experi == "EN_digits":
            filename = EN_digits_silence_files[i]
        elif experi == "digits":
            filename = CN_digits_silence_files[i]
        else:
            raise("No such experiment")
        num_button = 1
        try: 
            input_array = np.load("processed_datasets/{}/{}.npy".format(experi,filename.split("/")[-1].replace(".json", "")))
        except:
            input_array = raw_to_mfcc(filename, experi)
        input_list.append(input_array)
        label_list.append(num_button - 1) # need to minus oen to act as label.
        
        if experi == "EN_digits":
            filename = EN_digits_one_files[i]
        elif experi == "digits":
            filename = CN_digits_one_files[i]
        else:
            raise("No such experiment")
        num_button = 2
        try: 
        ...
        if experi == "EN_digits":
            filename = EN_digits_two_files[i]
        elif experi == "digits":
            filename = CN_digits_two_files[i]
        else:
            raise("No such experiment")
        num_button = 3
        try: 
        ...
        if experi == "EN_digits":
            filename = EN_digits_three_files[i]
        elif experi == "digits":
            filename = CN_digits_three_files[i]
        else:
            raise("No such experiment")
        num_button = 4
        try: 
        ...
        if experi == "EN_digits":
            filename = EN_digits_four_files[i]
        elif experi == "digits":
            filename = CN_digits_four_files[i]
        else:
            raise("No such experiment")
        num_button = 5
        try: 
        ...
        if experi == "EN_digits":
            filename = EN_digits_five_files[i]
        elif experi == "digits":
            filename = CN_digits_five_files[i]
        else:
            raise("No such experiment")
        num_button = 6
        try: 
        ...
        if experi == "EN_digits":
            filename = EN_digits_unknown_files[i]
        elif experi == "digits":
            filename = CN_digits_unknown_files[i]
        else:
            raise("No such experiment")
        num_button = 7
        try: 
...
# Train the device
if experiment == 'digits':
    train_in, train_out = getSamplesIIDDigits(samples_per_device, 0, 'digits')
    test_in, test_out = getSamplesIIDDigits(total_samples - samples_per_device, samples_per_device, 'digits')
...
if "EN_digits" in source_experiment:
    old_task_in, old_task_out = getSamplesIIDDigits(700, 6300)


if model_type == "fc":
    train_in = np.reshape(train_in, (samples_per_device, 650))
    test_in = np.reshape(test_in, (total_samples - samples_per_device, 650))
    old_task_in = np.reshape(old_task_in, (700, 650))
elif model_type == "conv1d":
    ...
    old_task_in = np.reshape(old_task_in, (700, 13, 50))
    ...
    old_task_in = np.reshape(old_task_in, (700, 1, 13, 50))
    # test_in = np.reshape(test_in, (total_samples - samples_per_device, 50, 13)).transpose(0, 2, 1)


old_val_error, old_val_accu = server_validate(old_task_in, old_task_out)

logger.debug(f"Original Old Task Validation Accuracy is {100 * old_val_accu}%\n")
...
for epoch in range(epoch_size):
    ...
    for batch in range(total_round):
        ...
        logger.debug(f"New Task Validation Accuracy {100 * val_accu}%\n")

        old_val_error, old_val_accu = server_validate(old_task_in, old_task_out)

        logger.debug(f"Old Task Validation Accuracy {100 * old_val_accu}%\n")

        if val_accu + old_val_accu > max_accu:
            max_accu = val_accu + old_val_accu
...
logger.debug("Final Best Combined (New + Old) Accuracy: {}.".format(max_accu))
...
plot_graph()
figname = f"newplots/centralizeCL_ES{epoch_size}-BS{batch_size}-LR{learningRate}-M{momentum}-NH{number_hidden}-HS{hidden_size}-HN{size_hidden_nodes}-TT{train_time}-{experiment}_train.eps"
...
plt.rc('figure', titlesize=font_xl)   # fontsize of the figure title
plot_val_graph()
figname2 = f"newplots/centralizeCL_ES{epoch_size}-BS{batch_size}-LR{learningRate}-M{momentum}-NH{number_hidden}-HS{hidden_size}-HN{size_hidden_nodes}-TT{train_time}-{experiment}_val.eps"
...
plt.rc('figure', titlesize=font_xl)   # fontsize of the figure title
plot_train_accu()
figname3 = f"newplots/centralizeCL_ES{epoch_size}-BS{batch_size}-LR{learningRate}-M{momentum}-NH{number_hidden}-HS{hidden_size}-HN{size_hidden_nodes}-TT{train_time}-{experiment}_train_accu.eps"
